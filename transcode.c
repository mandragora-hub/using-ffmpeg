...
static int initialize_input_streams(AVFormatContext *format_ctx,
  ...
  for (unsigned int i = 0; i < format_ctx->nb_streams; ++i) {
    if (format_ctx->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
      ...
      ivs->frame = av_frame_alloc();
      ivs->pkt = av_packet_alloc();
      continue;
    }
    if (format_ctx->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
      audio_stream_idx = i;
      ias->stream_idx = audio_stream_idx;
      ...
      //
      // Allocate frame and packet
      ias->frame = av_frame_alloc();
      ias->pkt = av_packet_alloc();
      continue;
    }
  }
...
static void close_input_stream(AVFormatContext *format_ctx,
                               InputStream *input_st) {
  // Free allocated resources
  // av_frame_free(&input_st->frame);
  avcodec_free_context(&input_st->codec_ctx);
}
...
int main(int argc, char **argv) {
  ...
  if (!oc) {
    ...
    avformat_alloc_output_context2(&oc, NULL, "mpeg", output_filename);
  }
  if (!oc) return 1;
  ...
  if (fmt->video_codec != AV_CODEC_ID_NONE) {
    add_stream(&video_st, oc, &video_codec, fmt->video_codec, &input_video_st,
               input_format_ctx);
    have_video = 1;
    encode_video = 1;
  }
  if (fmt->audio_codec != AV_CODEC_ID_NONE) {
    add_stream(&audio_st, oc, &audio_codec, fmt->audio_codec, &input_audio_st,
               input_format_ctx);
    have_audio = 1;
    encode_audio = 1;
  }
  ...
  while (av_read_frame(input_format_ctx, packet) >= 0) {
    if (packet->stream_index == input_video_st.stream_idx) {
      ...
      while (ret >= 0) {
        ret = avcodec_receive_frame(input_video_st.codec_ctx,
                                    input_video_st.frame);
        if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
          break;
        } else if (ret < 0) {
        ...
        video_st.tmp_frame = input_video_st.frame;
  ...
  // close input streams
  close_input_stream(input_format_ctx, &input_video_st);
  close_input_stream(input_format_ctx, &input_audio_st);
